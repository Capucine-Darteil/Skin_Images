...
from pipeline.model import initialize_dumb_model, compile_model, train_model, evaluate_model

df = pd.read_csv("/Users/capucinedarteil/code/Capucine-Darteil/Skin_Images/raw_data/sample.csv", index_col=0)
...
def preprocessing():
    ...
    df_processed = pd.DataFrame(preproc().fit_transform(df_sample), columns = preproc.get_feature_names_out())
    ...
    loss, accuracy, recall, precision = evaluate_model(X_test,y_test)
